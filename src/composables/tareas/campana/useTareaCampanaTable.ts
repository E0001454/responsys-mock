...
export function useTareaCampanaTable(props: {
  selectedFilters: { lineas: number[]; campanas: number[]; status: boolean[] }
  filteredTareas: TareaCampanaTableRow[]
  isLoading: boolean
}) {
  ...
  const countStageHorarios = (t: TareaCampanaTableRow, stageId: 1 | 2 | 3) => {
  ...
  const getStageInfo = (t: TareaCampanaTableRow, stageId: 1 | 2 | 3) => {
  ...
  const getStageVisual = (t: TareaCampanaTableRow, stageId: 1 | 2 | 3) => {
