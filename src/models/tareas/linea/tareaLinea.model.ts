...
export interface TareaLineaUpdateOperations {
  create: Array<{ stage: StageName; payload: CreateTareaLineaPayload }>
  update: Array<{ stage: StageName; payload: UpdateTareaLineaPayload }>
}
...
export function toCreateTareaLineaPayloads(
...
export function toUpdateTareaLineaOperations(
  form: TareaLineaFormModel,
  taskIds: TareaLineaStageTaskIds,
  activityTypeIdsInput?: Partial<ActivityTypeIds>
): TareaLineaUpdateOperations {
  const activityTypeIds = resolveActivityTypeIds(activityTypeIdsInput)
  const mapeoId = Number(form.idMapeo ?? 0)
  ...
  const create: Array<{ stage: StageName; payload: CreateTareaLineaPayload }> = []
