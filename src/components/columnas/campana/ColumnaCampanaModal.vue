...
const missingRequiredOptions = computed<Option[]>(() => {
	const selectedMapeo = String(form.value.idABCConfigMapeoCampana ?? '')
		...
		<template #body>
			<form @submit.prevent="requestSave" class="flex flex-col min-h-0 flex-1 h-full">
				<div class="p-3 sm:p-4 overflow-y-auto custom-scrollbar bg-slate-50 flex-1 min-h-0">
					...
					<div>
					<label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Requerido</label>
					<div class="flex items-center gap-3">
						<label class="inline-flex items-center gap-2">
							<input type="checkbox" v-model="form.esRequerido" class="h-4 w-4 accent-[#00357F]" />
							<span class="text-sm text-slate-600">Marcado si es requerido</span>
							</label>
						</div>
					</div>
