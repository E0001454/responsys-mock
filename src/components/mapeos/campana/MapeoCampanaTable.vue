...
interface Props {
  ...
  filteredMapeos: MapeoCampanaData[]
  totalMapeos: number
  currentPage: number
  totalPages: number
  canPrevPage: boolean
...
interface Emits {
  ...
  (e: 'viewDetails', item: MapeoCampanaData): void
  (e: 'viewColumnas', item: MapeoCampanaData): void
  (e: 'toggleStatus', item: MapeoCampanaData): void
  (e: 'edit', item: MapeoCampanaData): void
...
const { isRowGlowing } = useFirstRowNewGlow(
  ...
  row => Number(row.idABCConfigMapeoLinea ?? 0),
  {
    ...
    getRowChangeToken: row => {
      const item = row as MapeoCampanaData
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible flex flex-col min-h-[72dvh] h-[calc(100dvh-11rem)] max-h-[calc(100dvh-8rem)] max-[650px]:h-[78dvh] max-[650px]:min-h-[68dvh] max-[650px]:max-h-none">
    <div class="overflow-y-auto overflow-x-hidden max-[1300px]:overflow-x-auto flex-1 min-h-0">
      <table class="w-full max-[1300px]:min-w-[1240px] text-left border-collapse table-fixed">
        <colgroup>
          <col class="w-[10%]" />
          <col class="w-[10%]" />
          <col class="w-[17%]" />
